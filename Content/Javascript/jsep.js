...
jsep.plugins.register(newPlugin);
...
class Executor {
	...
	evaluate(node, context = null) {
		switch (node.type) {
			...
			case 'Literal':
				return node.value;
			case 'Identifier':
				return this.identifier(node, context);
			case 'CallExpression':
			...
			case 'MemberExpression':
				return this.memberExpression(node, context);
			case 'NewExpression':
	...
	identifier(node, context) {
		...
		} else if (this.variables.hasOwnProperty(node.name)) {
			return this.variables[node.name];
		} else if (this.functions.hasOwnProperty(node.name)) {
			return this.functions[node.name];
		} else if (this.types.hasOwnProperty(node.name)) {
			return this.types[node.name];
		} else {
			throw new Error(`Undefined variable or function: ${node.name}`);
		}
	}
	...
	logicalExpression(node) {
		const left = this.evaluate(node.left);
		switch (node.operator) {
			case '&&': return left && this.evaluate(node.right);
			case '||': return left || this.evaluate(node.right);
			default:
				throw new Error(`Unsupported operator: ${node.operator}`);
		}
	...
	callExpression(node, context) {
		const func = this.evaluate(node.callee, context);
	...
	memberExpression(node, context) {
		...
		const property = node.computed ? this.evaluate(node.property) : node.property.name;
		if (object === undefined || object === null) {
			throw new Error('Cannot read property of undefined or null');
		}
	...
	newExpression(node) {
		...
		if (typeof constructor !== 'function') {
			throw new Error(`Unsupported type: ${node.callee.name}`);
		}
		const args = node.arguments.map(arg => this.evaluate(arg));
...
executor.addFunction('sum', (a, b) => a + b);
// 添加常用数学函数
Object.getOwnPropertyNames(Math).forEach(name => {
	if (typeof Math[name] === 'function') {
		executor.addFunction(name, Math[name]);
	}
});
// 添加String原型上的所有方法
Object.getOwnPropertyNames(String.prototype).forEach(name => {
	if (typeof String.prototype[name] === 'function') {
		executor.addFunction(name, function (...args) {
			const context = this;
			return String.prototype[name].apply(context, args);
		});
	}
});
